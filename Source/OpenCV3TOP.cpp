...
void
OpenCV3TOP::getGeneralInfo(TOP_GeneralInfo* ginfo)
{
...
bool
OpenCV3TOP::getOutputFormat(TOP_OutputFormat* format)
{
...
void
OpenCV3TOP::execute(const TOP_OutputFormatSpecs* outputFormat,
	OP_Inputs* inputs,
	TOP_Context *context)
{
	myExecuteCount++;
...
int32_t
OpenCV3TOP::getNumInfoCHOPChans()
{
...
void
OpenCV3TOP::getInfoCHOPChan(int32_t index, OP_InfoCHOPChan* chan)
{
	...
	if (index == 2) {
		chan->name = "face_detected";
		chan->value = (float)faceDetected;
	}

	if (index == 3)
	{
		chan->name = "minScaledSize";
		chan->value = (float)minScaledSize;
	}

	if (index == 4) {

		chan->name = "maxScaledSize";
...
OpenCV3TOP::getErrorString() {
	...
	if (myErrors == 1) {
		...
		return "Face Cascade XML File is missing";

	}
	/*
	else if (myErrors == 2) {

		return "Eyes Cascade XML File is missing";
...
#endif
			entries->values[i+1] = tmpBuffer[i];
		}
	}
...
void
OpenCV3TOP::setupParameters(OP_ParameterManager* manager)
{
...
void
OpenCV3TOP::pulsePressed(const char* name)
{
